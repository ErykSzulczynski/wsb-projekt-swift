    ...
    var body: some View {
        Form {
            Section(header: Text("Zadanie")) {
                TextField("Nazwa zadania", text: $name)
                TextField("Opis", text: $desc)
            }
            
            Section(header: Text("Termin wykonania zadania")) {
                Toggle("Czas zegarowy", isOn: $scheduleTime)
                DatePicker("Termin", selection: $dueDate, displayedComponents: displayComps())
            }
            
            if selectedTaskItem?.isCompleted() ?? false {
                Section(header: Text("Wykonano")) {
